...
from photutils import (aperture_photometry, detect_sources,
                      # DAOStarFinder, deblend_sources, detect_threshold,
                       CircularAperture, CircularAnnulus, source_properties)
import warnings
...
def detect_sources_segmap(data, threshold, npixels, kernel_fwhm=1.8,
    ...
    threshold : float or array
        Detection threshold value, or pixel-wise threshold
        image (must be same shape as `data`.)
    npixels : int
        Positive integer number of connected pixels, each
        greater that `threshold` that an object must have
        to be detected.
    kernel_fwhm : float
        FWHM of gaussian kernel used to smooth image before
        segmentation.
    show : bool
    ...
    coo_tab : `astropy.table.Table` or int
        Table with detected source(s). Returns '0' if no
